...
class RechnungDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        if self._is_finalized:
            warning = QFrame()
            warning.setObjectName("rechnungWarning")
        ...
        scroll = QScrollArea()
        scroll.setObjectName("rechnungDialogScroll")
        scroll.setWidgetResizable(True)
        scroll.setFrameShape(QFrame.Shape.NoFrame)
        scroll.setStyleSheet("#rechnungDialogScroll { background: transparent; }")
    ...
    def _build_header(self) -> QFrame:
        header = QFrame()
        header.setObjectName("rechnungDialogHeader")
        header.setFixedHeight(64)
        header.setStyleSheet(f"""
            #rechnungDialogHeader {{
    ...
    def _build_content(self) -> QWidget:
        content = QWidget()
        content.setObjectName("rechnungDialogContent")
        content.setStyleSheet("#rechnungDialogContent { background: transparent; }")
    ...
    def _build_summen_widget(self) -> QFrame:
        frame = QFrame()
        frame.setObjectName("rechnungSummen")
    ...
    def _build_footer(self) -> QFrame:
        footer = QFrame()
        footer.setObjectName("rechnungDialogFooter")
        footer.setFixedHeight(72)
        footer.setStyleSheet(f"""
            #rechnungDialogFooter {{
                background-color: {Colors.BG_SURFACE};
    ...
    def _populate(self, dto: RechnungDTO) -> None:
        ...
        # MwSt — gespeicherten Wert auswählen (ggf. hinzufügen wenn nicht in Liste)
        mwst_text = f"{dto.mwst_prozent:,.2f} %".replace(".", ",")
        idx = self._mwst_combo.findText(mwst_text)
        if idx >= 0:
            self._mwst_combo.setCurrentIndex(idx)
        else:
            # Historischer MwSt-Satz (z.B. 16% aus Corona-Zeit) → hinzufügen
