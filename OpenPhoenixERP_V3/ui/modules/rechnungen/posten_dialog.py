"""
ui/modules/rechnungen/posten_dialog.py – Dialog für Rechnungsposten
====================================================================
Zum Hinzufügen und Bearbeiten einzelner Rechnungsposten.
Unterstützt Artikelauswahl aus dem Stamm.
"""
...
from core.services.rechnungen_service import PostenDTO, berechne_gesamtpreis
...
class PostenDialog(QDialog):
    """
    Dialog zum Anlegen / Bearbeiten eines Rechnungspostens.

    Signals:
        saved(PostenDTO): Wird ausgelöst wenn der Nutzer speichert.
    """

    saved = Signal(object)  # PostenDTO
    ...
    def __init__(
        ...
        dto: PostenDTO = None,
        position: int = 1,
        artikel_liste: list = None,  # list of (artikelnummer, beschreibung, einheit, preis)
    ):
    ...
    def _build_ui(self) -> None:
        ...
        self.f_beschreibung = FormField(
            "Beschreibung *", required=True, placeholder="Leistung oder Produkt beschreiben"
        )
        ...
        self.f_artikelnummer = FormField("Artikelnummer", placeholder="z.B. ART-001")
        self.f_einheit = FormField("Einheit", placeholder="z.B. Stück, h, kg")
        self.f_einheit.setFixedWidth(140)
    ...
    def _populate(self, dto: PostenDTO) -> None:
    ...
    def _update_gesamtpreis(self, *_) -> None:
        ...
        self._gesamtpreis_label.setText(f"{gesamt:,.2f} €".replace(",", "X").replace(".", ",").replace("X", "."))

    def _collect(self) -> PostenDTO:
        menge = self._parse_decimal(self.f_menge.value())
        preis = self._parse_decimal(self.f_einzelpreis.value())
        return PostenDTO(
            id=self._original_dto.id if self._original_dto else None,
            rechnung_id=self._original_dto.rechnung_id if self._original_dto else None,
