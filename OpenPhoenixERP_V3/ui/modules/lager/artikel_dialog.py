...
class ArtikelDialog(QDialog):
    ...
    def _build_form(self) -> QWidget:
        container = QWidget()
        container.setObjectName("artikelDialogForm")
        container.setStyleSheet("#artikelDialogForm { background: transparent; }")
    ...
    def _collect(self) -> ArtikelDTO:
        preis_str = self.f_preis.value().replace(".", "").replace(",", ".")
        try:
        ...
        if self.f_bestand:
            bestand_str = self.f_bestand.value().replace(".", "").replace(",", ".")
            try:
