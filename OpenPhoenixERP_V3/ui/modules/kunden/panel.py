...
class KundenDetailPanel(QScrollArea):
    ...
    def _build_ui(self) -> None:
        container = QWidget()
        container.setObjectName("kundenDetailContainer")
        container.setStyleSheet(f"#kundenDetailContainer {{ background-color: {Colors.BG_APP}; }}")
        self._detail_container = container
        ...
        self._info_label_pairs: dict[str, tuple] = {}
        fields = [
            ("Adresse",   "adresse"),
            ("Telefon",   "telefon"),
            ("E-Mail",    "email"),
        ...
        for label_text, key in fields:
            ...
            self._info_label_pairs[key] = (lbl, val)
            row.addWidget(lbl)
            row.addWidget(val, 1)
    ...
    def _on_theme_changed(self, _mode: str) -> None:
        ...
        self._detail_container.setStyleSheet(
            f"#kundenDetailContainer {{ background-color: {Colors.BG_APP}; }}"
        )
        self._kundennr_label.setStyleSheet(f"color: {Colors.TEXT_SECONDARY};")
        self._line.setStyleSheet(f"background-color: {Colors.BORDER};")
        self._info_frame.setStyleSheet(f"""
            QFrame[frameRole="surface"] {{
                background-color: {Colors.BG_SURFACE};
                border-radius: {Radius.LG}px;
        ...
        """)
        for key, (lbl, _val) in self._info_label_pairs.items():
            lbl.setStyleSheet(f"color: {Colors.TEXT_SECONDARY};")
...
class KundenPanel(QWidget):
    ...
    def _build_header(self) -> QFrame:
        header = QFrame()
        header.setObjectName("kundenPanelHeader")
    ...
    def _build_toolbar(self) -> QWidget:
        toolbar = QWidget()
        toolbar.setObjectName("kundenToolbar")
        toolbar.setStyleSheet("#kundenToolbar { background: transparent; }")
