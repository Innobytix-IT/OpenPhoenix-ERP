...
class _SmtpWorker(QObject):
    """Hintergrund-Worker für nicht-blockierenden SMTP-Versand."""

    finished = Signal(bool, str)  # (success, message)

    def __init__(
        self, server_host: str, port: int, user: str, password: str,
        encryption: str, msg, parent=None,
    ):
    ...
    def run(self) -> None:
        """Wird im QThread ausgeführt — blockiert NICHT den UI-Thread."""
        try:
            if self._encryption == "SSL/TLS":
                server = smtplib.SMTP_SSL(self._server_host, self._port, timeout=30)
            else:
                server = smtplib.SMTP(self._server_host, self._port, timeout=30)
            ...
            try:
                ...
                server.quit()
            self.finished.emit(True, "E-Mail erfolgreich gesendet.")
...
class DokumenteDialog(QDialog):
    ...
    def _email_smtp_dialog(self, doc: DokumentDTO) -> None:
        ...
        def do_send():
            ...
            # MIME-Nachricht vorbereiten (im Main-Thread — nur I/O-frei)
            msg = self._build_mime_message(to, subject, body, doc.dokument_pfad)

            # SMTP-Versand in Hintergrund-Thread
            ...
            self._smtp_worker = _SmtpWorker(
                server_host, port, user, pw, encryption, msg,
            )
            self._smtp_worker.moveToThread(self._smtp_thread)
            self._smtp_thread.started.connect(self._smtp_worker.run)

            def on_finished(success: bool, message: str):
                # Kein quit()/wait() hier – das erledigen die Signal-Ketten unten,
                # um einen Deadlock zu vermeiden (wait() im Signal-Handler blockiert).
                if success:
                    self._banner.show_success(f"E-Mail an {to} erfolgreich gesendet.")
                    dlg.accept()
                else:
                    self._banner.show_error(f"Fehler beim Senden: {message}")
                    btn_send.setEnabled(True)
                    btn_cancel.setEnabled(True)
                    btn_send.setText("Senden")

            # Signal-Ketten: Worker fertig → UI-Callback + Thread stoppen + Cleanup
            self._smtp_worker.finished.connect(on_finished)
            self._smtp_worker.finished.connect(self._smtp_thread.quit)
            self._smtp_worker.finished.connect(self._smtp_worker.deleteLater)
    ...
    def _build_mime_message(
        ...
        """Baut die MIME-Nachricht inkl. Anhang (im Main-Thread, kein I/O-Wait)."""
        user = config.get("smtp", "user", "")
        msg = MIMEMultipart()
        msg["From"]    = user
        msg["To"]      = to
