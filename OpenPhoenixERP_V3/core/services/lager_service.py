...
class LagerService:
    ...
    def artikel_aktualisieren(
        ...
        # Artikelnummer darf nicht geändert werden wenn in Rechnungen/Angeboten verwendet
        if artikel.artikelnummer != dto.artikelnummer.strip():
            from core.models import Rechnungsposten, AngebotsPosten
            in_use = (
                session.query(Rechnungsposten)
                .filter_by(artikelnummer=artikel.artikelnummer)
                .first()
            )
            if not in_use:
                in_use = (
                    session.query(AngebotsPosten)
                    .filter_by(artikelnummer=artikel.artikelnummer)
                    .first()
                )
            if in_use:
                return ServiceResult.fail(
                    ...
                    "da der Artikel bereits in Rechnungen/Angeboten verwendet wird."
                )
    ...
    def einbuchen(
        ...
        # with_for_update() sperrt die Zeile gegen parallele Änderungen (PostgreSQL)
