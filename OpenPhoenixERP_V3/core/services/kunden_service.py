...
class KundenService:
    ...
    def deaktivieren(self, session: Session, kunde_id: int) -> ServiceResult:
        ...
        Verweigert die Deaktivierung, wenn noch offene Rechnungen existieren,
        damit der Kunde nicht aus dem Mahnwesen verschwindet.
        """
        kunde = session.get(Kunde, kunde_id)
        if not kunde:
        ...
        if offene > 0:
            return ServiceResult.fail(
                f"Kunde kann nicht deaktiviert werden: "
                f"Es bestehen noch {offene} offene Rechnung(en). "
                f"Bitte zuerst alle Rechnungen abschließen oder stornieren."
            )
    ...
    def notiz_hinzufuegen(
        ...
        session.flush()
        audit.log(
            session, AuditAction.NOTIZ_ERSTELLT,
            record_id=notiz.id,
            table_name="kunden_notizen",
            details=f"Notiz für Kunde #{kunde_id} erstellt: {text[:80]}",
        )
        return ServiceResult.ok(
    ...
    def notiz_loeschen(self, session: Session, notiz_id: int) -> ServiceResult:
        ...
        kunde_id = notiz.kunde_id
        audit.log(
            session, AuditAction.NOTIZ_GELOESCHT,
            record_id=notiz_id,
            table_name="kunden_notizen",
            details=f"Notiz #{notiz_id} für Kunde #{kunde_id} gelöscht.",
        )
