...
def _esc(text: str) -> str:
    """Escaped Freitext für ReportLab Paragraph (XML-sicher).

    ReportLab's Paragraph interpretiert den Text als XML/HTML.
    Zeichen wie < > & in Benutzereingaben (z.B. "Müller & Co",
    "Länge < 5m") würden sonst zum XML-Parse-Crash führen.
    """
    if not text:
        return ""
...
class PDFService:
    ...
    def _story(self, dto, s, w, session=None):
        ...
        rows = [kopf]
        for p in dto.posten:
            bez = _esc(p.beschreibung or "")
            if getattr(p, "artikelnummer", None):
                bez += f"\n{_esc(p.artikelnummer)}"
            rows.append([
                str(p.position), bez,
        ...
        # Context-Werte XML-escapen, damit Sonderzeichen in Kundennamen
        # (z.B. "Müller & Co.", "Länge < 5m") ReportLab nicht crashen.
        _ctx_safe = {k: _esc(str(v)) for k, v in _ctx.items()}
        _hint = resolve(_vorlagen.get("rechnung_hinweis", ""), _ctx_safe)
        if not _hint:
        ...
        if bank:
            _hint += f"<br/><b>Bankverbindung:</b> {_esc(bank)}"
    ...
    def _build_mahnung(self, buf, dto, konfig):
        ...
        try:
            ...
            # Session bleibt offen — wird im finally-Block unten geschlossen
            logger.debug(f"Kundendaten für Mahnung nicht ladbar: {e}")
    ...
    def _mahnung_story(self, dto, s, w, konfig, session=None):
        ...
        # Context-Werte XML-escapen gegen Sonderzeichen in Kundennamen etc.
        _ctx_safe = {k: _esc(str(v)) for k, v in _ctx.items()}
        _k_titel, _k_text, _k_schluss = vorlagen_key_fuer_status(dto.status)
        _intro = resolve(_vorlagen.get(_k_text, ""), _ctx_safe)
        ...
        _schluss = resolve(_vorlagen.get(_k_schluss, ""), _ctx_safe)
        bank = config.get("company", "bank_details", "")
        if bank:
            story.append(Paragraph(
                f"Bitte überweisen Sie auf unser Konto:", s_normal))
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph(_esc(bank).replace("\n", "<br/>"), s_bold))
        ...
        if _schluss:
            ...
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph(f"<b>{_esc(c_firma)}</b>", s_normal))
    ...
    def _kundenadresse(self, dto, session) -> dict:
        ...
        try:
            ...
            logger.debug(f"Kundendaten für Platzhalter nicht ladbar: {e}")
        return result
