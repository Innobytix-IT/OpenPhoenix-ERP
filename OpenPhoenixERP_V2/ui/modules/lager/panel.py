...
class LagerPanel(QWidget):
    ...
    def _build_stats_row(self) -> QWidget:
        ...
        row = QWidget()
        row.setStyleSheet(f"background-color: {Colors.BG_SURFACE}; border-bottom: 1px solid {Colors.BORDER};")
        ...
        self._stat_labels = {}
        stats = [
            ("aktiv",    "Aktive Artikel",  Colors.TEXT_PRIMARY),
            ("kritisch", "Kritischer Bestand", Colors.WARNING),
            ("negativ",  "Negativbestand",  Colors.ERROR),
            ("lagerwert","Lagerwert (Netto)", Colors.INFO),
        ]
