...
from core.models import Rechnung, Rechnungsposten, Artikel, Kunde, AuditLog
...
class RechnungsService:
    ...
    def naechste_rechnungsnummer(
        ...
        prefix = f"{jahr}-"
        result = (
            session.query(func.max(Rechnung.rechnungsnummer))
        ...
        if result:
            try:
                laufende_nr = int(result.split("-")[1]) + 1
        ...
        try:
            ...
            for i, p in enumerate(dto.posten, start=1):
                posten = Rechnungsposten(
                    ...
                    position=i,
                    artikelnummer=p.artikelnummer.strip() or None,
        ...
        try:
            ...
            for posten in rechnung.posten:
                if posten.artikelnummer:
                    ...
                    if artikel:
                        alte_menge = artikel.verfuegbar
                        artikel.verfuegbar = Decimal(str(artikel.verfuegbar)) - Decimal(str(posten.menge))
                        lager_log.append(
                            f"{posten.artikelnummer}: {alte_menge} → {artikel.verfuegbar}"
                        )
    ...
    def skonto_gewaehren(
        ...
        Reduziert offener_betrag und summe_brutto.
        """
        rechnung = session.get(Rechnung, rechnung_id)
        if not rechnung:
            return ServiceResult.fail("Rechnung nicht gefunden.")
    ...
    def teilgutschrift_erstellen(
        ...
        if rechnung.status == RechnungStatus.STORNIERT:
            return ServiceResult.fail("Stornierte Rechnungen können nicht korrigiert werden.")

        try:
            mwst_betrag = (netto * mwst_prozent / Decimal("100")).quantize(Decimal("0.0001"))
            ...
            gutschrift = Rechnung(
                ...
                summe_netto=-netto.quantize(Decimal("0.0001")),
                summe_mwst=-mwst_betrag,
                summe_brutto=-brutto.quantize(Decimal("0.0001")),
                mahngebuehren=Decimal("0"),
            ...
            gp = Rechnungsposten(
                ...
                menge=Decimal("-1"),
                einheit="Stück",
                einzelpreis_netto=netto.quantize(Decimal("0.0001")),
                gesamtpreis_netto=-netto.quantize(Decimal("0.0001")),
            )
            session.add(gp)
    ...
    def stornieren(
        ...
        try:
            ...
            # 2. Storno-Nummer generieren (fortlaufend, GoBD-konform)
            storno_nummer = self._naechste_storno_nummer(session)

            # 3. Gutschrift anlegen
            gutschrift = Rechnung(
                kunde_id=rechnung.kunde_id,
                ...
                summe_brutto=-(rechnung.summe_brutto or Decimal("0")),
                mahngebuehren=Decimal("0"),
                offener_betrag=Decimal("0"),
                status=RechnungStatus.GUTSCHRIFT,
                bemerkung=(
            ...
            for p in rechnung.posten:
                ...
                if p.artikelnummer:
                    ...
                    if artikel:
                        artikel.verfuegbar = Decimal(str(artikel.verfuegbar)) + Decimal(str(p.menge))
